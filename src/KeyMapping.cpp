...
KeyMappingEntry KeyMapping::jsonToEntry(const QJsonObject &obj) const
{
    ...
    entry.filterRepeats = obj["filterRepeats"].toBool(true);  // Default to true for backward compatibility
    entry.suppressRepeats = obj["suppressRepeats"].toBool(false);  // Default to false for backward compatibility

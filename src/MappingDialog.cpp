...
namespace {
    constexpr int DEFAULT_CHANNEL = 0;          // MIDI channel 1 (0-based)
    constexpr int DEFAULT_NOTE = 60;            // Middle C
    constexpr int DEFAULT_VELOCITY = 100;       // Strong velocity
    constexpr int DEFAULT_CONTROLLER = 1;       // Modulation wheel
    constexpr int DEFAULT_VALUE = 127;          // Maximum value
...
void MappingDialog::setupUI()
{
    ...
    m_filterRepeatsCheck->setChecked(true);  // Default to enabled
    ...
    m_suppressRepeatsCheck->setChecked(false);  // Default to disabled
...
void MappingDialog::setupKeyUpGroup()
{
    ...
    m_keyUpTypeCombo->setCurrentIndex(1); // Default to Note Off
...
KeyMappingEntry MappingDialog::getMappingEntry() const
{
    ...
    int vkCode = m_vkCodeEdit->text().toInt(&ok);
    if (!ok || vkCode < 1 || vkCode > 255) {
        return entry; // Return empty entry if invalid
    }
...
{
    bool isNoteMessage = (m_keyDownTypeCombo->currentIndex() == 0 || m_keyDownTypeCombo->currentIndex() == 1); // Note On or Note Off
    bool isControlChange = (m_keyDownTypeCombo->currentIndex() == 2); // Control Change
...
{
    bool isNoteMessage = (m_keyUpTypeCombo->currentIndex() == 0 || m_keyUpTypeCombo->currentIndex() == 1); // Note On or Note Off
    bool isControlChange = (m_keyUpTypeCombo->currentIndex() == 2); // Control Change
